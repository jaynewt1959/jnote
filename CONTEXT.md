...
src/
  components/
    ...
    FeedbackBanner.jsx      ✓/✗ result + reaction time per answer
    HintLabel.jsx           Fixed-height text hint below staff (never causes layout shift)
    LevelProgress.jsx       Level N of 12 + comfortable count + progress bar
...
Each level adds notes cumulatively. A level advances when all notes in the pool reach score ≥ 4 (4 net correct identifications). Phase 2 (accidentals) is not yet implemented.
...
### Spaced repetition
Scores 0–5 per note. Correct: +1 (cap 5). Wrong: −1 (floor 0). Weighted random selection: `weight = (6 − score)²`. Level advances when entire pool reaches score ≥ 4. Full state in `localStorage` key `jnote_state`.
...
Timer starts in `showNextNote` (`noteStartTime = Date.now()`). RT is computed in `submitAnswer` and stored in `_state.rtLog[]` (capped at 500). Only correct answers under 15s count toward stats. `getStats()` returns `{ avgMs, improvementMs }` comparing last 10 vs prior 10 correct answers.
